...
```{r}
ggplot(df.bs.query.critical, aes( x = condition))+
...
### Ridge Plots
```{r}
ggplot(df.query.critical.pretty, 
...
### Stats
```{r}
model.data <- df.query.critical %>% select(workerid, predicate_1, val, condition)
# dummy coded with NP as comparison level
fit.coord.brm <- brm(val ~ condition + (1 + condition | workerid) + (1 + condition | predicate_1), model.data)
summary(fit.coord.brm)
```
