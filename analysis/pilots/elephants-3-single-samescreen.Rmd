...
df.expt1.query.critical <- df.expt1.query %>%
  ...
  mutate(me = if_else(str_detect(condition1, "bugs"), "nme", "me"),
         interrupted = if_else(str_detect(condition1, "and"), "uninterrupted", "interrupted")) %>%
...
df.query.critical.full <- full_join(df.query.critical %>%
                                      mutate(question_type = "samescreen"),
                                    df.expt1.query.critical %>% select(-condition1) %>% 
                                      filter(
                                        key == '% Africa', condition != 'nme_interrupted'
                                      )
                                    ) %>%
  full_join(df.expt1.single.query.critical) %>% 
...
fit.expt1 <- lmer(val ~ condition + (1 | workerid) + (1 | predicate_1), model.data)
summary(fit.expt1)
```
